  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    Flux<RSocket> connectionFlux = Flux.create((FluxSink<RSocket> sink) -> {
      rsocketMono.subscribe(rSocket -> {
        rSocket.onClose().log("ON_CLOSE: ").doOnTerminate(() -> sink.error(new IOException("Disconnected"))).subscribe();
        sink.next(rSocket);
    ...
    connectionFlux.flatMap(rsocket -> rsocket.requestResponse(DefaultPayload.create("hola")).log("REQ_RESP: "))
        .subscribe();
